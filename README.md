...
1. 按本文启动项目；
2. 电脑访问 `/health`，确认返回 `status: ok`；
3. 电脑打开移动端首页，完成角色选择；
4. 发送消息，确认显示发送中动画；
5. 确认收到“底座已连通”；
6. 停止后端后尝试发送消息，确认显示失败提示和“重新发送”；
7. 重启后端并点击“重新发送”，确认恢复；
8. 按“手机同网段访问”步骤在真机重新走通一次；
9. 执行 `go test ./...`，确认全部测试通过。

## 已知限制

- 聊天接口当前返回固定文案，尚未连接知识库和大模型；
- 身份选择仅用于 MVP 演示，不提供真实账号认证；
- 当前消息只在页面内展示，尚未写入 SQLite；
- 当前没有文档上传、RAG 引用或 LIMS 查询能力。
