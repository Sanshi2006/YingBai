...
面向纺织品、鞋类和消费品检测实验室的移动端智能客服底座。当前已完成移动端与 Gin 后端联通，以及知识文档上传、文本提取、语义切片、千问向量化、pgvector 持久化、Top-K 向量检索、完整 RAG 回答、SQLite 对话日志、最近 5 轮会话记忆和 LIMS 订单查询 Mock。
...
- 首次空状态、明确加载文案、重试中状态和基于 `visualViewport` 的软键盘高度适配；
- `GET /health` 健康检查；
...
- `/chat` 按 `sessionId` 和当前角色从 SQLite 恢复最近 5 轮问答，支持连续追问；
- 移动端将 `sessionId` 保存在 `localStorage`，页面刷新后继续沿用原会话；
- 自然语言识别订单号并查询只读 Mock LIMS，固定有效订单为 `ORD2026001`；
- 移动端以卡片展示订单号、状态、检测进度和报告状态，并提供查订单、常见标准、转人工快捷语；
- Gin 同源托管移动端页面，无需单独安装或启动前端工具；
- 同一局域网内可通过手机浏览器或微信内置浏览器访问。

当前角色入口是演示级身份选择，不是生产级登录认证。完整 RAG 链路、最近 5 轮多轮记忆和只读 LIMS Mock 已接入 `/chat`，移动端可展开查看引用文件名或查看结构化订单卡片。
...
├─ docs/底座说明.md         # 架构、数据流、边界和后续建议
├─ docs/测试用例.md         # 18 条自动化与真机验收用例
...
6. 点击“查订单”快捷语或输入“查一下订单 ORD2026001”，查看 Mock 订单卡片。
...
先在电脑浏览器打开实际局域网地址，例如 `http://192.168.1.20:8080`。如果浏览器地址栏提供“为此页面创建二维码”或“创建 QR 码”，可以生成后让手机扫码；二维码中必须是电脑 IPv4 地址，不能是 `127.0.0.1` 或 `localhost`。没有二维码功能时直接在手机输入地址即可。
...
自然语言中包含订单号时会优先进入只读 LIMS Mock，不调用向量或 Chat LLM。固定有效订单 `ORD2026001` 返回 HTTP 200：

```json
{
  "answer": "已查询到订单 ORD2026001 的 Mock 进度信息。",
  "type": "order",
  "sessionId": "session-1",
  "role": "customer",
  "citations": [],
...
不存在的订单不会生成虚假状态、进度或报告信息，而是返回 `found: false` 和明确提示：

```json
{
  "answer": "未查询到订单 ORD9999999，请核对订单号后重试。",
  "type": "order",
  "sessionId": "session-1",
  "role": "customer",
  "citations": [],
  "order": {
    "found": false,
    "mock": true,
    "orderNumber": "ORD9999999"
  }
}
```

只输入“查订单”但未提供订单号时，会提示填写类似 `ORD2026001` 的订单号。Mock LIMS 查询异常时返回 HTTP 502 和稳定错误码 `LIMS_QUERY_FAILED`。
...
每次问答会在生成答案前读取同一 `sessionId`、同一角色最近 5 个已完成问答，并按时间顺序加入问题改写和回答上下文。历史只用于理解代词、省略和连续追问，最终结论仍只能来自本轮权限过滤后的知识片段。切换角色不会携带原角色历史；SQLite 历史读取失败时返回 HTTP 500 和稳定错误码 `CHAT_HISTORY_FAILED`。

移动端首次访问时生成 `sessionId` 并写入 `localStorage` 的 `lab-chat-session-id`。刷新页面后重新进入聊天，会继续提交同一标识，因此即使页面不重新绘制旧消息，后端仍能从 SQLite 恢复最近 5 轮上下文。清除该站点浏览数据后会创建新会话。

最近 5 轮问题和回答会发送到 `.env` 中 `LLM_CHAT_BASE_URL` 指向的外部 Chat LLM，用于理解连续追问。部署者必须确保这类会话历史的外部传输符合实际数据安全和合规要求。
...
- SQLite 按 `sessionId` 和角色读取最近 5 轮，按时间正序加入问题改写和回答 Prompt；
- 超过 5 轮时旧轮次被截断，角色切换时不会带入其他角色历史；
- 会话历史读取失败时停止模型调用并返回 `CHAT_HISTORY_FAILED`。
- 直接表达、带礼貌词和小写订单号均可触发 LIMS Mock；查单不调用大模型；
- `ORD2026001` 重复查询返回稳定结构，不存在订单返回 `found: false` 且不伪造业务字段；
- 三种角色均由后端校验，客户只检索公开知识，客服和管理员可以检索公开及内部知识；
- 移动端安全渲染 Mock 订单卡片，并在 375 像素宽度内展示订单号、状态、进度和报告状态。
- 移动端空状态、加载文案、重复提交保护、失败重试状态和 `visualViewport` 键盘适配结构。

完整的 18 条验收用例、前置条件、步骤、预期结果和证据位置见 [`docs/测试用例.md`](docs/测试用例.md)。系统架构、数据流和能力边界见 [`docs/底座说明.md`](docs/底座说明.md)。
...
- 已完成 SQLite 对话日志和最近 5 轮会话记忆；默认文件为 `data/runtime/conversations.db`，不提供面向移动端的日志查询接口；
- 已完成只读 LIMS Mock；当前不连接真实 LIMS，不修改订单，除 `ORD2026001` 外的订单均返回未找到；
- 当前向量列固定为 1024 维，更换为其他维度的模型前必须同步执行数据库迁移；
- 当前没有提供直接返回文本分块的公网接口，分块由后续检索模块在后端内部使用。
- 页面刷新后会续用 Session 和最近 5 轮上下文，但不会重新绘制刷新前的消息气泡；
- 角色选择是演示入口，不是生产账号认证或订单归属授权；
- LIMS 仅为固定只读 Mock，不连接真实业务系统；
- Android、iOS 和微信内置浏览器的软键盘行为仍需使用实际验收设备确认。

## 常见故障

- PostgreSQL 未就绪：执行 `docker compose ps`，确认 `postgres` 为 `healthy`。
- 8080 端口占用：按“修改端口”章节设置 `PORT=8090`，并同步修改访问地址。
- 模型返回 401/403：检查 `.env` 中对应的 `LLM_*` 或 `LLM_CHAT_*` 密钥、Base URL 和模型名，不要把密钥粘贴到日志或问题中。
- 上传成功但问答拒答：确认文档状态为 `vectorized`、角色有权访问、提问与文档相关，并检查相似度阈值。
- 手机无法访问：按“手机无法访问时”依次检查 Wi-Fi、IPv4、端口、防火墙、VPN 和路由器隔离。
- 键盘遮挡或页面高度未恢复：先收起键盘并刷新页面；记录手机型号、系统和浏览器版本后按 `docs/测试用例.md` 的 TC-15 复现。
